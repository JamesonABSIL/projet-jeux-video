...
function handleClick () {
  ...
  return (
    <>
      ...
      <div>        
        <div className="carousel max-w-full p-4 space-x-4 bg-neutral rounded-box">
          {parties.map((game : Igame) => (
            (date===formatedDateOnlyDay(game.beginAt) && game.status != "finished" &&
      ...
      >
        {open ?
        <button className="btn btn-outline btn-success z-50 bg-black" onClick={handleClick}>
          Créer une partie
        </button>
        : <button className="btn btn-outline btn-error z-50 bg-black" onClick={handleClick}>
          X
        </button>}
