...
export default function Genre() {
  ...
  const genre = useAppSelector((state) => findBySlug(state.genres.list, slug))
  const userFavorite = useAppSelector((state) => state.users.currentUser.favorites)
  
  return (
    <>
    {!genre ? <div className="flex justify-center"><img src='https://i.gifer.com/5IPd.gif' /></div> : <div> <h1>{genre.name}</h1>
    
    <div className='flex flex-wrap gap-5 justify-center'>
      {genre.videoGames.map((game : Ivideo_game)=> 
