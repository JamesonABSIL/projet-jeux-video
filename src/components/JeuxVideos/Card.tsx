...
export default function Card({
  ...
  const updateFavorite = async () => {
   const {data} = await axios.post(
    ...
    dispatch(addToFavorite(data.favorites))
  };
  ...
  return (
    ...
    <div className="card-body ">
      ...
      <div className="card-actions justify-end justify-between">
        {findBySlug(userFavorite, slug) ?
         <button className="btn coeur" onClick={handleClickRemove}> 
          <svg
        ...
        <button className="btn coeur" onClick={handleClickAdd}> 
        <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-10 w-10"
            fill={findBySlug(userFavorite, slug)  ? '#b91c1c' : 'none'}
